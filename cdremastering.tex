...
\end{lstlisting}
\todo{Quite a few things seem unclear. A lot of stuff is packed into packages,
but some stuff on TUn!x wasn't (e.g. vpnc.conf and screenrc). The installer
installs only (?) the packages listed in the manifest-desktop file - or not?
or does it also install files from the live-cd?}
